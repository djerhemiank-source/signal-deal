(()=>{
  ...
  function renderGeo(text,active=false){ensurePersonal();const box=document.getElementById('sdGeo');if(!box)return;box.innerHTML='<strong>📍 Opportunités près de vous</strong><br>'+text+'<div class="sd-note">Vos coordonnées GPS exactes ne sont pas enregistrées. Signal Deal mémorise uniquement le département et la région.</div><button id="sdGeoBtn" class="sd-btn '+(active?'good':'')+'" data-sd-action="locate">'+(active?'Actualiser ma zone':'Utiliser ma position')+'</button>'}
