(()=>{
  ...
  function decorate(){
    document.querySelectorAll('#feed .card').forEach(card=>{
      const o=findOpp(card);if(!o||!isRenewal(o))return;
      card.dataset.sdRenewal='1';

      const tags=card.querySelector('.tagrow');
      if(tags){
        const first=tags.querySelector('.tag');
        if(first&&!first.classList.contains('sd-preview'))first.textContent='DECP · CONTRAT ATTRIBUÉ';
      ...
      if(!card.querySelector('.sd-renewal-note')){
        ...
        if(signal)signal.insertAdjacentElement('afterend',note);
      ...
      if(btn){
        ...
        const chev=btn.querySelector('.sd-chevron');
        btn.childNodes.forEach(n=>{if(n.nodeType===Node.TEXT_NODE)n.textContent=''})
        btn.insertAdjacentText('afterbegin',p.label+' ');
  ...
  setInterval(decorate,900);setTimeout(decorate,400);
})();