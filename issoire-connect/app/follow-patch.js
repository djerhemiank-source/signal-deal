...
if(!(S.follows instanceof Set))S.follows=new Set(Array.isArray(S.follows)?S.follows.map(x=>typeof x==='string'?x:x?.business_id).filter(Boolean):[]);
if(!Array.isArray(S.followRows))S.followRows=[];
...
function rows(){if(!Array.isArray(S.followRows))S.followRows=[];return S.followRows}
function notifications(){if(!Array.isArray(S.notifications))S.notifications=[];return S.notifications}
function row(id){return rows().find(f=>f.business_id===id)}
function label(id){return S.follows.has(id)?'♥ Suivi':'♡ Suivre ce commerçant'}
function followButton(id,compact=false){return `<button class="btn ${S.follows.has(id)?'brand':''}" data-ic-follow="${E(id)}" onclick="event.stopPropagation();toggleFollow('${E(id)}')">${compact?(S.follows.has(id)?'♥ Suivi':'♡ Suivre'):label(id)}</button>`}
function syncButtons(id){document.querySelectorAll(`[data-ic-follow="${CSS.escape(String(id))}"]`).forEach(b=>{b.textContent=b.closest('.modal')?label(id):(S.follows.has(id)?'♥ Suivi':'♡ Suivre');b.classList.toggle('brand',S.follows.has(id))})}

loadPrivate=async function(){
 await _loadPrivateFollow();
 if(!S.session){S.followRows=[];S.follows=new Set();S.notifications=[];return}
 ...
 S.followRows=Array.isArray(f.data)?f.data:[];
 S.follows=new Set(S.followRows.map(x=>x.business_id).filter(Boolean));
...
window.toggleFollow=async function(id){
 ...
 if(existing||S.follows.has(id)){
  const {error}=await sb.from('ic_follows').delete().eq('user_id',uid).eq('business_id',id);
  if(error)return say(error.message);
  S.followRows=rows().filter(f=>f.business_id!==id);S.follows.delete(id);
  ...
  if(error)return say(error.message);
  rows().unshift(data);S.follows.add(id);syncButtons(id);say('Commerce ajouté à vos favoris. Choisissez maintenant les notifications souhaitées.');
...
window.saveFollowPreferences=async function(id){
 ...
 if(error)return say(error.message);
 S.followRows=rows().map(x=>x.business_id===id?data:x);closeModal();say('Préférences de notifications enregistrées.');
};

businessCard=function(b){
 let h=_businessCardFollow(b);
 if(!h.includes(`data-ic-follow="${b.id}"`))h=h.replace('</article>',`<div class="actions" style="margin-top:8px">${followButton(b.id,true)}${S.follows.has(b.id)?`<button class="btn" onclick="event.stopPropagation();openFollowPreferences('${E(b.id)}')">🔔 Régler</button>`:''}</div></article>`);
 return h;
};
viewBusiness=function(id){
 ...
 if(!modalBody.querySelector?.(`[data-ic-follow="${CSS.escape(String(id))}"]`))modalBody.insertAdjacentHTML('beforeend',`<div class="actions" style="margin-top:14px">${followButton(id)}${S.follows.has(id)?`<button class="btn" onclick="openFollowPreferences('${E(id)}')">🔔 Mes notifications</button>`:''}</div>`);
};
...
function followingSection(){
 ...
 const followed=rows().map(f=>businesses.find(b=>b.id===f.business_id)).filter(Boolean);
