...
function row(id){return S.follows.find(f=>f.business_id===id)}
...
window.saveFollowPreferences=async function(id){
 ...
 if(error)return say(error.message);
 S.follows=S.follows.map(x=>x.business_id===id?data:x);closeModal();say('Préférences de notifications enregistrées.');
};
...
viewBusiness=function(id){
 ...
 const b=S.businesses.find(x=>x.id===id);if(!b||!modalBody)return;
