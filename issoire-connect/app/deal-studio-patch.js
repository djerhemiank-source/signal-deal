...
window.saveDeal=async function(id){
 ...
 const starts=iso($('#dealStudioStart')?.value),ends=iso($('#dealStudioEnd')?.value),pickup=iso($('#dealStudioPickup')?.value),reservation=!!$('#dealStudioReservable')?.checked,isActive=!!$('#dealStudioActive')?.checked,pushRequested=!existing&&!!$('#dealStudioPush')?.checked;
 ...
 if(pushRequested&&!isActive)return say('Rendez l’offre visible immédiatement pour envoyer l’alerte téléphone / PC.');
 ...
 const btn=$('#dealStudioSave');if(btn){btn.disabled=true;btn.textContent=pushRequested?'Publication + alerte…':'Enregistrement…'}
 ...
 let resultMessage=existing?'Bon plan mis à jour.':'Bon plan publié.';
 if(!existing&&pushRequested){
   const {data:prepared,error:prepareError}=await sb.rpc('ic_prepare_offer_push',{p_offer_id:data.id});
   if(prepareError){resultMessage+=' Notification interne conservée, mais alerte téléphone/PC non envoyée : '+friendly(prepareError)}
   else{
     const prep=Array.isArray(prepared)?prepared[0]:prepared;
     if(prep?.alert_id&&typeof window.dispatchBusinessAlertPush==='function'){
       const pushed=await window.dispatchBusinessAlertPush(prep.alert_id);
       if(pushed?.sent>0)resultMessage+=` Alerte téléphone/PC envoyée sur ${Number(pushed.sent)} appareil${Number(pushed.sent)>1?'s':''}.`;
       else if(pushed?.already_dispatched)resultMessage+=' L’alerte téléphone/PC avait déjà été envoyée.';
       else if(pushed?.in_progress)resultMessage+=' L’envoi téléphone/PC est déjà en cours.';
       else resultMessage+=` ${Number(prep.recipient_count||0)} abonné${Number(prep.recipient_count||0)>1?'s':''} ciblé${Number(prep.recipient_count||0)>1?'s':''}, mais aucun appareil Push actif n’a pu être joint.`;
     }
   }
 }
 closeModal();say(resultMessage);if(typeof loadPrivate==='function')await loadPrivate();proAccount();return data;
};
