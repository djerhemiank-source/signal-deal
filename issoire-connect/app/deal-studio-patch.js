...
function openEditor(o,bid){
 ...
 const pushChoice=edit?'':`<label><input id="dealStudioPush" type="checkbox"> 🔔 Envoyer aussi l’alerte téléphone / PC</label><div class="muted">Dans la limite quotidienne du forfait, les abonnés ayant activé cette catégorie reçoivent la notification dans Issoire Connect. Cette option envoie en plus la notification système sur leurs appareils abonnés.</div>`;
...
window.saveDeal=async function(id){
 ...
 if(!existing&&pushRequested){
   ...
   if(prepareError){resultMessage+=' Aucune alerte supplémentaire n’a été envoyée : '+friendly(prepareError)}
   else{
