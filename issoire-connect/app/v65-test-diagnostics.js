(function(){
'use strict';
const BUILD='2026-09-06-v65-tests';
...
window.markLeakedPasswordV65=async function(flag){
 if(!S.v59?.isAdmin||!sb)return say('Administrateur requis');
 if(flag){
  if(!confirm('Confirmez uniquement si vous avez réellement activé « Leaked Password Protection » dans Supabase Auth. Cette validation sera journalisée.'))return;
 }else if(!confirm('Marquer cette vérification comme non validée ?'))return;
 const {data,error}=await sb.rpc('ic_admin_set_security_verification',{p_key:'auth_leaked_password_protection',p_verified:!!flag});
 if(error)return say(error.message);
 say(flag?'Vérification sécurité enregistrée':'Vérification sécurité réinitialisée');
...
if(typeof previousAdminPageV65==='function')window.admin59Page=function(){
 ...
 main.insertAdjacentHTML('beforeend',`<section class="section"><div class="card"><div class="section-head"><div><span class="v61-tag">V.65 QA</span><h3>🔐 Validation sécurité de release</h3><p>Après activation réelle de la protection des mots de passe compromis dans Supabase Auth, enregistrez la vérification ici. Le release gate reste bloqué tant que ce point, l’identité légale et un Checkout TEST réel ne sont pas validés.</p></div></div><div class="actions"><button class="btn green" onclick="markLeakedPasswordV65(true)">✅ Je l’ai activée dans Supabase</button><button class="btn" onclick="markLeakedPasswordV65(false)">Réinitialiser</button><button class="btn primary" onclick="runCloudDiagV65()">🧪 Tester les accès cloud</button></div><div id="v65CloudResult" class="muted" style="margin-top:10px"></div></div></section>`);
};
console.info('Issoire Connect V.65 diagnostics',BUILD);
})();
